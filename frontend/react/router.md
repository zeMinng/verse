...
React Router v6 的核心变化是采用 ==createBrowserRouter== 配置式路由（替代 v5 的 `Switch`/`Route` 组件式写法），通过 `RouterProvider` 挂载。关键概念：`<Link>` 声明式导航 vs `useNavigate` 编程式导航、`useSearchParams` 读取 query 参数、`useParams` 读取路径参数、`<Outlet>` 渲染嵌套路由。默认二级路由通过 `index: true` 配置，404 路由通过 `path: '*'` 实现。路由懒加载结合 `lazy()` + `<Suspense>` 可有效减小首屏包体积。

::: info 📖 相关资源
- [React Router 官方文档](https://reactrouter.com/) - 核心概念、API 详解及使用示例
- [React Router v6 教程](https://reactrouter.com/en/main/start/tutorial) - 官方入门教程
:::
