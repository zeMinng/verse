...
图片管理与引入是我们前端开发中非常常见的需求，合理高效地引入图片不仅能提升项目性能，还能优化开发体验。本篇介绍图片引入方式：如模板、CSS、JavaScript 动态加载，以及批量动态导入的高阶用法。

此文来源于 [掘金](https://mp.weixin.qq.com/s/d5kkCc5Cai3SaEV1sviEjg)。


::: tip
如果是 uniapp 项目，你应该看看 [官方文档](https://uniapp.dcloud.net.cn/tutorial/page-static-assets.html)。
:::
...
- **src/assets 文件夹：**适用于需要经过 Vite 打包处理的图片资源，支持动态导入、路径别名 (@) 等功能。一般情况，我们项目中的图片资源都放这里。

- **public 文件夹：**用于存放静态资源，直接映射到构建后的根目录中，适合无需打包的图片，如 logo 或外部提供的资源。
...
/**
 * 这是因为 @ 别名（指向 src/ 目录）在字符串模板中不会被解析。
 * Vite 的别名 @ 仅适用于 import 或动态导入的语法（如 import.meta.glob 或 new URL），
 * 而在普通的字符串中，@ 只是一个普通字符，不会被识别为路径别名。
 */
</script>
```

```vue [❌ 错误案例：通过 src/ 导入]
<!-- 这样写确实在开发环境能够展示图片，但这不是最佳实践 ！ -->
...
```javascript [使用 import.meta.glob]
...
<script setup>
/**
 * @description 基于 Vite 的模块 glob 导入函数，用于批量匹配并导入符合模式的文件
 * @param {string | string []} pattern - 匹配文件路径的通配符模式
 * @param {object} [options] - 导入选项
 * @returns {object} 返回值是一个对象，键为文件路径，值为导入函数或导入的内容（取决于 eager 配置）。
 * 
 * @param {boolean} [options.eager=false] - 是否立即加载文件（true 表示立即加载）。
 * @param {string} [options.as] - 指定导出内容的格式: 例如 raw（导出文件内容为字符串）。
 */
