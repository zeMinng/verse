...
Redux Toolkit 是目前使用 Redux 的标准方式，通过 `createSlice` 简化了 action 和 reducer 的定义，内置 Immer 支持"可变"写法。核心流程为：==createSlice 定义状态和 reducers → configureStore 组装 store → Provider 挂载 → useSelector/useDispatch 在组件中使用==。异步操作可通过 `createAsyncThunk` 或在 `createSlice` 外定义 thunk 函数处理。对于新项目，如果不需要 RTK Query 的数据缓存能力，Zustand 通常是更轻量的选择。

::: info 📖 相关资源
- [Redux Toolkit 官方文档](https://redux-toolkit.js.org/) - Redux Toolkit 完整教程
- [React Redux 文档](https://react-redux.js.org/) - React 绑定库
- [Zustand](https://github.com/pmndrs/zustand) - 轻量级状态管理替代方案
:::
