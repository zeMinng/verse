...
所有权是 Rust 最独特的特性 — 没有垃圾回收器却能保证内存安全，靠的是编译器在编译期追踪每块内存的归属。

## 一、所有权基础

Rust 中，在默认所有权模型下，每个值都有且仅有一个所有者；当所有者离开作用域时，值会被自动释放。可以用图书馆借书来类比：

- 同一本书只能有一个人借走（唯一所有者），所有权转移后，原所有者不再可用；
- 所有者离场后，书会自动归架，对应 Drop 释放；
- 其他人可以同时借阅，但不能批注，对应不可变借用 &T；
- 如果要批注，同一时间只能有一人借走，对应可变借用 &mut T。

这三条规则共同构成了 Rust 内存安全的基石。
...
fn main() {
  ...
  let len = calc_len(&s); // &s 借用 s，不转移所有权
...
引用分为两种：`&T` 是不可变借用，可同时存在多个；`&mut T` 是可变借用，同一时刻只能存在一个。**针对同一份数据，可变借用与不可变借用不能在同一作用域内重叠。**
...
单一所有者 + 借用已经能满足大多数场景：父级拥有数据，子级借用访问。但有些值需要被多处同时持有 — 例如多个模块共享同一份配置、多个 UI 组件引用同一个数据源。为此，Rust 提供了引用计数类型，如 `Rc<T>`（单线程）和 `Arc<T>`（多线程），用于共享所有权。若还需要在共享的同时修改数据，可以再配合 `RefCell<T>`、`Mutex<T>` 或 `RwLock<T>` 使用。
