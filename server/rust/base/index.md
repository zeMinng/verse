...
Rust 是静态类型语言，每个变量的类型都在编译期确定。这不是刁难，而是保护：编译器能提前发现类型不匹配，避免程序在运行时崩溃。

初学阶段重点关注 6 类常用类型：整数（默认 i32）、浮点（默认 f64）、布尔、字符，以及字符/字符串（String / &str）。其余类型用到时再查即可。**注意：Rust 不会隐式转换数值类型**。 i32 不能直接赋给 i64，必须用 as 显式转换。
...
Rust 既支持显式标注类型，也能根据上下文自动推导。初学阶段，建议对关键变量显式标注，便于阅读和排查错误。

`String` 与 `&str` 的区别： String 是分配在堆上的可变字符串，拥有数据所有权，可以增删改；&str 是字符串切片，不拥有数据，只是指向已有字符串的“视图”。

初学时可记：字符串字面量用 `&str`（如 let s = "hello"），需要修改或拼接时用 `String`（如 String::from("hello")）。
...
复合类型可以把多个值组织成一个整体。Rust 中常用的复合类型有元组、数组和切片：元组适合存放少量、类型可以不同的数据；数组适合存放“同类型、固定长度”的数据；切片（slice）则是对连续内存的引用视图，不拥有数据所有权，`&str` 和 `&[T]` 是最常见的两种切片。
...
参数与返回值类型需要显式标注。Rust 强调**表达式风格**：函数体最后一行不带分号即为返回值，加了分号就变成语句返回 `()`（单元类型）。需要返回多个值时，可以用元组打包。
